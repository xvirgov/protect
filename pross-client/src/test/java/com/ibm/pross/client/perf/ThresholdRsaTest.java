...
public class ThresholdRsaTest {
    ...
    final int total_iterations = 110;
    final int startIter = 10;
    ...
    @Test
    public void testGenerateRsaKeyPair() throws IOException, NoSuchAlgorithmException, InvalidKeySpecException, InterruptedException {

        File file1 = new File("rsa-keypair-time.csv");
        file1.delete();
        ...
        for (int s = 0; s < primes.size(); s = s + 2) {
            ...
            for (int i = 0; i < total_iterations; i++) {
                ...
                // compute modulus
                BigInteger modulus = primes.get(s).multiply(primes.get(s + 1));
                ...
                final BigInteger sqrtG = RandomNumberGenerator.generateRandomInteger(modulus);
                final BigInteger g = sqrtG.modPow(BigInteger.valueOf(2), modulus);
                end = System.nanoTime();
                ...
                assertTrue(realD.multiply(e).mod(totient).equals(BigInteger.ONE));
            }
            System.out.println(accu.divide(BigInteger.valueOf(iterations)));
        }
    }

    @Test
    public void testComputeAdditiveShares() throws IOException {
        File file1 = new File("rsa-additive-shares-time.csv");
        file1.delete();

        for (int c = 0; c < numServersChoice.size(); c++) {
            int numServers = numServersChoice.get(c);
    ...
    @Test
    public void testComputePublicVerificationValues() throws IOException {
        File file1 = new File("rsa-pub-verif-time.csv");
        file1.delete();
    ...
    @Test
    public void testComputePolynomialVSSvaluesForEachAdditiveShare() throws IOException {
        File file1 = new File("rsa-vss-additive-time.csv");
        file1.delete();

        for (int c = 0; c < numServersChoice.size(); c++) {
            int numServers = numServersChoice.get(c);
            for (int t = 0; t < thresChoice.size(); t++) {
                int threshold = (int) (numServers * thresChoice.get(t));

                System.out.println("NumServers: " + numServers + ", threshold: " + threshold);
    ...
    public void testOverallRsAKeyGen() throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {
        File file1 = new File("rsa-gen-overall.csv");
        file1.delete();

...
/**
 * This class represents a state transfered from a replica to another. The state
 * associated with the last checkpoint together with all the batches of messages
 * received do far, comprises the sender's current state
 * 
 * @author Joao Sousa
 */
public class DefaultApplicationState implements ApplicationState {
	...
	/**
	 * Constructs a TansferableState This constructor should be used when there is a
	 * valid state to construct the object with
	 * 
	...
	/**
	 * Constructs a TansferableState This constructor should be used when there
	 * isn't a valid state to construct the object with
	 */
	...
	/**
	 * Retrieves the consensus ID for the last messages batch delivered to the
	 * application
	 * 
	 ...
	 * 
	 * @param cid
	 *            Consensus ID associated with the batch to be fetched
	 ...
	 * 
	 * @return Consensus ID for the last checkpoint, or -1 if no checkpoint was yet
	 *         executed
	 */
	...
	public int hashCode() {
		...
		if (this.stateHash != null) {
			for (int i = 0; i < this.stateHash.length; i++)
				hash = hash * 31 + (int) this.stateHash[i];
		} else {
			hash = hash * 31 + 0;
		}

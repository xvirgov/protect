 ...
 * instead of all replicas send the state and log, or state and hash of the log,
 * the replicas are divided into three groups. One group sends the state,
 * another group sends the lower half of the log and the third group sends the
 * upper portion of the log. The replica that receives the state, logs and
 * hashes validates the state and install it. The details of the protocol are
 * described in the paper "On the Efficiency of Durable State Machine
 * Replication".
 *
...
public abstract class DurabilityCoordinator implements Recoverable, BatchExecutable {
	...
	private static final Logger logger = LogManager.getLogger(DurabilityCoordinator.class);
	...
	private byte[][] executeBatch(byte[][] commands, MessageContext[] msgCtx, boolean noop) {
		...
		if (checkpointIndex == -1) {
			...
			logger.debug(
					"(DurabilityCoordinator.executeBatch) Storing message batch in the state log for consensus " + cid);
			saveCommands(commands, msgCtx);
		} else {
			// there is a replica supposed to take the checkpoint. In this case, the
			// commands
			// has to be executed in two steps. First the batch of commands containing
			// commands
			...
			if (secondHalf.length > 0) {
				...
				System.arraycopy(msgCtx, checkpointIndex + 1, secondHalfMsgCtx, 0,
			...
			if (cid % globalCheckpointPeriod == replicaCkpIndex && lastCkpCID < cid) {
				logger.debug("(DurabilityCoordinator.executeBatch) Performing checkpoint for consensus " + cid);
				stateLock.lock();
			...
			if (secondHalf.length > 0) {
				// logger.debug("----THERE IS A SECOND HALF----");
	...
	/**
	 * Iterates over the commands to find if any replica took a checkpoint. When a
	 * replica take a checkpoint, it is necessary to save in an auxiliary table the
	 * position in the log in which that replica took the checkpoint. It is used
	 * during state transfer to find lower or upper log portions to be restored in
	 * the recovering replica. This iteration over commands is needed due to the
	 * batch execution strategy introduced with the durable techniques to improve
	 * state management. As several consensus instances can be executed in the same
	 * batch of commands, it is necessary to identify if the batch contains
	 * checkpoint indexes.
	 * 
	 * @param msgCtxs
	 *            the contexts of the consensus where the messages where executed.
	 ...
	 * 
	 * @param msgCtx
	 *            the message context of the commands executed by the replica. There
	 *            is one message context for each command
	 * @param cid
	 *            the CID of the consensus where a replica took a checkpoint
	...
	public int setState(ApplicationState recvState) {
		...
		if (recvState instanceof CSTState) {
			...
			logger.debug("--- Installing log from " + (lastCheckpointCID + 1) + " to " + lastCID);
	 ...
	 * 
	 * @param commands
	 *            array of commands. Each command is an array of bytes
	 * @param msgCtx
	 */
	private void saveCommands(byte[][] commands, MessageContext[] msgCtx) {
		...
		for (int i = 0; i <= msgCtx.length; i++) {
			if (i == msgCtx.length) { // the batch command contains only one command or it is the last position of the
				...
				// if(batchStart > 0)
				// logger.debug("Last batch: " + commands.length + "," + batchStart + "-"
				// + i + "," + batch.length);
			} else {
				if (msgCtx[i].getConsensusId() > cid) { // saves commands when the CID changes or when it is the last
					...
					byte[][] batch = Arrays.copyOfRange(commands, batchStart, i);
					MessageContext[] batchMsgCtx = Arrays.copyOfRange(msgCtx, batchStart, i);
					// logger.debug("THERE IS MORE THAN ONE CID in this batch." +
					// commands.length + "," + batchStart + "-" + i + "," + batch.length);
					log.addMessageBatch(batch, batchMsgCtx, cid);
					log.setLastCID(cid, globalCheckpointPeriod, checkpointPortion);
	...
	public void setReplicaContext(ReplicaContext replicaContext) {
		...
		if (log == null) {
			...
			if (config.isToLog()) {
				...
				// log = new DurableStateLog(replicaId, state, computeHash(state), isToLog,
				// syncLog, syncCkp);
				...
				if (storedState.getLastCID() > -1) {
					logger.debug("LAST CID RECOVERED FROM LOG: " + storedState.getLastCID());
					setState(storedState);
					getStateManager().setLastCID(storedState.getLastCID());
				} else {
					logger.debug("REPLICA IS IN INITIAL STATE");
				}
			}
	 ...
	 * logging the only information important in MessageContext is the consensus id,
	 * it saves time to have it already in an array of ids
	 * 
	 * @param ctxs
	 *            the message context, one for each command to be executed
	...
	public byte[] getCurrentStateHash() {
		...
		logger.debug(
				"--- State size: " + currentState.length + " Current state Hash: " + Arrays.toString(currentStateHash));

...
/**
 * This class manages consensus instances. It can have several epochs if there
 * were problems during consensus.
 *
 * @author Alysson
 */
public final class ExecutionManager {
	...
	// ******* EDUARDO BEGIN: now these variables are all concentrated in the
	// Reconfigurationmanager **************//
	// private int me; // This process ID
	// private int[] acceptors; // Process ID's of all replicas, including this one
	// private int[] otherAcceptors; // Process ID's of all replicas, except this
	// one
	// ******* EDUARDO END **************//
	...
	// Paxos messages that were out of context (that didn't belong to the consensus
	// that was/is is progress
	private Map<Integer, List<ConsensusMessage>> outOfContext = new HashMap<Integer, List<ConsensusMessage>>();
	// Proposes that were out of context (that belonged to future consensuses, and
	// not the one running at the time)
	...
	// When the execution manager is stopped, incoming paxos messages are stored
	// here
	 ...
	 * @param controller
	 * @param acceptor
	 *            Acceptor role of the PaW algorithm
	 * @param proposer
	 *            Proposer role of the PaW algorithm
	 * @param me
	 *            This process ID
	 */
	...
	public void stop() {
		...
		if (tomLayer.getInExec() != -1) {
			...
			if (stoppedEpoch != null)
				logger.debug("(ExecutionManager.stop) Stoping epoch " + stoppedEpoch.getTimestamp() + " of consensus "
						+ tomLayer.getInExec());
		}
	...
	/**
	 * Checks if this message can execute now. If it is not possible, it is stored
	 * in outOfContextMessages
	 *
	 * @param msg
	...
	public final boolean checkLimits(ConsensusMessage msg) {
		...
		logger.debug("(ExecutionManager.checkLimits) I'm at consensus " + inExec + " and my last consensus is "
				+ lastConsId);
		...
		if (isRetrievingState || // Is this replica retrieving a state?
				(!(lastConsId == -1 && msg.getNumber() >= (lastConsId + revivalHighMark)) && // not a recovered replica
						(msg.getNumber() > lastConsId && (msg.getNumber() < (lastConsId + paxosHighMark))) && // not an
																												// ahead
																												// of
																												// time
																												// message
						!(stopped && msg.getNumber() >= (lastConsId + timeoutHighMark)))) { // not a timed-out replica
																							// which needs to fetch the
																							// state

			if (stopped) {// just an optimization to avoid calling the lock in normal case
				stoppedMsgsLock.lock();
				if (stopped) {
					logger.debug("(ExecutionManager.checkLimits) adding message for consensus " + msg.getNumber()
							+ " to stoopped");
				...
				if (isRetrievingState || msg.getNumber() > (lastConsId + 1)
						...
						|| (inExec == -1 && msg.getType() != PaxosMessageType.PROPOSE)) { // not propose message for the
																						// next consensus
					logger.debug("(ExecutionManager.checkLimits) Message for consensus " + msg.getNumber()
					...
					// isRetrievingState="+isRetrievingState);
				...
				(stopped && msg.getNumber() >= (lastConsId + timeoutHighMark))) { // or a timed-out replica which needs
																					// to fetch the state
			...
			if (controller.getStaticConf().isStateTransferEnabled()) {
				...
				System.out
						.println("##################################################################################");
				logger.debug("- Ahead-of-time message discarded");
				logger.debug("- If many messages of the same consensus are discarded, the replica can halt!");
				logger.debug("- Try to increase the 'system.paxos.highMarc' configuration parameter.");
				logger.debug("- Last consensus executed: " + lastConsId);
				System.out
						.println("##################################################################################");
			}
			/******************************************************************/
	...
	/**
	 * Informs if there are messages till to be processed associated the specified
	 * consensus
	 * 
	 * @param cid
	 *            The ID for the consensus in question
	 ...
	 * 
	 * @param id
	 *            ID of the consensus to be removed
	 ...
	 *
	 * @param cid
	 *            ID of the consensus to be returned
	...
	public void processOutOfContext(Consensus consensus) {
		...
		if (messages != null) {
			logger.debug("(ExecutionManager.processOutOfContext) (" + consensus.getId() + ") Processing other "
					+ messages.size() + " out of context messages.");

			for (Iterator<ConsensusMessage> i = messages.iterator(); i.hasNext();) {
				acceptor.processMessage(i.next());
				if (consensus.isDecided()) {
					logger.debug(
							"(ExecutionManager.processOutOfContext) consensus " + consensus.getId() + " decided.");
					break;
				}
			}
	...
	/**
	 * Stores a message established as being out of context (a message that doesn't
	 * belong to current executing consensus).
	 *
	 * @param m

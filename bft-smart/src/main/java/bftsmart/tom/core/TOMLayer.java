...
public final class TOMLayer extends Thread implements RequestReceiver {
	...
	private static final Logger logger = LogManager.getLogger(TOMLayer.class);
	 ...
	 *
	 * @param data
	 *            Data from which to generate the hash
	 ...
	 *
	 * @param inEx
	 *            ID of the consensus being executed at the moment
	 */
	 ...
	 *
	 * @param dec
	 *            Object that will eventually hold the decided value
	...
	public TOMMessage[] checkProposedValue(byte[] proposedValue, boolean addToClientManager) {
		...
		try {
			...
			for (TOMMessage r : requests) {
				if (controller.getStaticConf().isBFT() && !verifier.isValidRequest(r))
					return null;
			}

...
public class ServiceProxy extends TOMSender {
	...
	private static final Logger logger = LogManager.getLogger(ServiceProxy.class);
	/**
	 * Constructor
	 *
	 * @see bellow
	...
	public byte[] invoke(byte[] request, TOMMessageType reqType) {
		...
		if (requestType == TOMMessageType.UNORDERED_HASHED_REQUEST) {
			...
			logger.info("[" + this.getClass().getName() + "] replyServerId(" + replyServer + ") " + "pos("
		...
		if (response == null) {
			...
			if (reqType == TOMMessageType.ORDERED_REQUEST) {
				...
				if (response.getViewID() > getViewManager().getCurrentViewId()) {
					...
					logger.info("Unexpected execution flow");
				}
			}
		}
		// ******* EDUARDO END **************//
	...
	public void replyReceived(TOMMessage reply) {
		logger.info("Synchronously received reply from " + reply.getSender() + " with sequence number "
				+ reply.getSequence());
		...
		try {
			...
			if (reply.getSequence() == reqId && reply.getReqType() == requestType) {
				...
				logger.info("Receiving reply from " + reply.getSender() + " with reqId:" + reply.getSequence()
						+ ". Putting on pos=" + pos);
				...
				logger.info("Ignoring reply from " + reply.getSender() + " with reqId:" + reply.getSequence()
	...
	private class HashResponseController {
		...
		public TOMMessage getResponse(int pos, TOMMessage tomMessage) {
			...
			logger.info("[" + this.getClass().getName() + "] hashReplies[" + pos + "]="

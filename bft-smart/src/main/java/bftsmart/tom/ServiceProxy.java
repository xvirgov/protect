...
public class ServiceProxy extends TOMSender {
		...
		try {
			if (reqType == TOMMessageType.UNORDERED_HASHED_REQUEST) {
				...
				if (!this.sm.tryAcquire(invokeTimeout, TimeUnit.SECONDS)) {
					...
					logger.debug("Reply timeout for reqId=" + reqId);
					logger.debug(getProcessId() + " // " + reqId + " // TIMEOUT // ");
					logger.debug("Replies received: " + receivedReplies);
					canSendLock.unlock();
	...
	public void replyReceived(TOMMessage reply) {
		...
		try {
			...
			logger.debug("Problem at ServiceProxy.ReplyReceived()");
			ex.printStackTrace();
			canReceiveLock.unlock();
		}
	}

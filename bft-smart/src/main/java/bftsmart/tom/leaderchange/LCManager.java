...
public class LCManager {
	 ...
	 *
	 * @param reconfManager
	 *            The reconfiguration manager from TOM layer
	 * @param md
	 *            The message digest engine from TOM layer
	 */
	public LCManager(TOMLayer tomLayer, ServerViewController SVController, MessageDigest md) {
		...
		try {
			this.mac = Mac.getInstance(SVController.getStaticConf().getHmacAlgorithm());
	 ...
	 *
	 * @param requestsFromSTOP
	 *            Requests received in a STOP message
	 */
	 ...
	 * 
	 * @param ts
	 *            timestamp up to which to discard messages
	 */
	 ...
	 * 
	 * @param regency
	 *            Regency up to which to discard information
	 */
	 ...
	 *
	 * @param collects
	 *            the collect data to which to apply the predicate.
	...
	public boolean sound(HashSet<CollectData> collects) {
		...
		logger.debug("(LCManager.sound) I collected the context from " + collects.size() + " replicas");
		...
		for (CollectData c : collects) { // organize all existing timestamps and values separately
			...
			logger.debug("(LCManager.sound) Context for replica " + c.getPid() + ": CID["
		...
		for (int r : timestamps) {
			for (byte[] v : values) {
				...
				logger.debug("(LCManager.sound) testing predicate BIND for timestamp/value pair ("
						+ r + " , " + Arrays.toString(v) + ")");
		...
		if (unbound) {
			logger.debug("(LCManager.sound) Predicate UNBOUND is true for N-F replicas");
			logger.debug(
					"(LCManager.sound) Predicate SOUND is true for the for context collected from N-F replicas");
		}
	...
	public boolean binds(int timestamp, byte[] value, HashSet<CollectData> collects) {
		...
		if (value == null || collects == null) {
			logger.debug("(LCManager.binds) Received null objects, returning false");
			return false;
		}

		if (!(collects.size() >= (SVController.getCurrentViewN() - SVController.getCurrentViewF()))) {
			logger.debug("(LCManager.binds) Less than N-F contexts collected from replicas, returning false");
			return false;
		}
	 ...
	 * 
	 * @param collects
	 *            Set of collects from which to determine the value
	...
	public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
		...
		if (appears)
			logger.debug("(LCManager.quorumHighest) timestamp/value pair (" + timestamp + " , "
	...
	public boolean certifiedValue(int timestamp, byte[] value, HashSet<CollectData> collects) {
		...
		for (CollectData c : collects) {
			...
			for (TimestampValuePair pv : c.getWriteSet()) {
				...
				// logger.debug("\t\t[CERTIFIED VALUE] " + pv.getTimestamp()
				// + " >= " + timestamp);
				// logger.debug("\t\t[CERTIFIED VALUE] " +
		...
		if (certified)
			logger.debug("(LCManager.certifiedValue) timestamp/value pair (" + timestamp + " , "
	 ...
	 * 
	 * @param regency
	 *            the regency from which the collects were stored
	 * @param cid
	 ...
	 * 
	 * @param signedObjects
	 *            original set of signed collects
	 * @param cid
	 *            the CID to which to normalize the collects
	 * @return a set of correctly signed and normalized collect data structures
	 */
		...
		for (ConsensusMessage consMsg : ConsensusMessages) {
			...
			if (consMsg.getProof() instanceof HashMap) { // Certificate is made of MAC vector
				...
				if (recvMAC != null && myMAC != null && Arrays.equals(recvMAC, myMAC)
						&& Arrays.equals(consMsg.getValue(), hashedValue) && consMsg.getNumber() == cDec.getCID()
				...
				logger.debug("(LCManager.hasValidProof) Proof is message is invalid");
			}
		}
	 ...
	 *            The consensus ID
	 * @param collects
	 *            The collects from the other replicas

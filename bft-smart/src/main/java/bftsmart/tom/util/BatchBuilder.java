 ...
 * + N_MESSAGES*[MSGSIZE(int),MSG(byte),SIG(byte)] +
 *
 *
...
public final class BatchBuilder {
	...
	private static final Logger logger = LogManager.getLogger(BatchBuilder.class);
	...
	private byte[] createBatch(long timestamp, int numberOfNonces, long seed, int numberOfMessages,
			int totalMessagesSize, boolean useSignatures, byte[][] messages, byte[][] signatures,
			ServerViewController controller) {
		int size = 20 + // timestamp 8, nonces 4, nummessages 4
				(numberOfNonces > 0 ? 8 : 0) + // seed if needed
				(numberOfMessages * (4 + (useSignatures ? TOMUtil.getSignatureSize(controller) : 0))) + // msglength +
																										// signature for
																										// each msg
	...
	public byte[] makeBatch(List<TOMMessage> msgs, int numNounces, long timestamp, ServerViewController controller) {
		...
		for (TOMMessage msg : msgs) {
			...
			logger.info(
					"(BatchBuilder.makeBatch) adding request from client " + msg.getSender() + " with sequence number "
							+ msg.getSequence() + " for session " + msg.getSession() + " to PROPOSE");

...
public class ClientsManager {
	...
	/**
	 * We are assuming that no more than one thread will access the same clientData
	 * during creation.
	 *
	 *
	 * @param clientId
	...
	/**
	 * Get pending requests in a fair way (one request from each client queue until
	 * the max number of requests is obtained).
	 *
	...
	public RequestList getPendingRequests() {
		...
		for (int i = 0; true; i++) {
			...
			while (it.hasNext() && allReq.size() < controller.getStaticConf().getMaxBatchSize()
	...
	/**
	 * We've implemented some protection for individual client data, but the clients
	 * table can change during the operation.
	 *
	...
	/**
	 * Notifies the ClientsManager that a new request from a client arrived. This
	 * method updates the ClientData of the client request.getSender().
	 *
	 * @param request
	 *            the received request
	 * @param fromClient
	 *            the message was received from client or not?
	 * @param storeMessage
	 *            the message should be stored or not? (read-only requests are not
	 *            stored)
	 * @param cs
	 *            server com. system to be able to send replies to already processed
	 *            requests
	 *
	 * @return true if the request is ok and is added to the pending messages for
	 *         this client, false if there is some problem and the message was not
	 *         accounted
	 */
	public boolean requestReceived(TOMMessage request, boolean fromClient, ServerCommunicationSystem cs) {
		...
		if (controller.getStaticConf().isBFT() && !verifier.isValidRequest(request))
			return false;
		...
		if ((clientData.getLastMessageReceived() == -1) || // first message received or new session (see above)
			...
			if (!request.signed
					|| clientData.verifySignature(request.serializedMessage, request.serializedMessageSignature)) {
	...
	/**
	 * Cleans all state for this request (e.g., removes it from the pending requests
	 * queue and stop any timer for it).
	 *
	 * @param request
	 *            the request ordered by the consensus
	 */

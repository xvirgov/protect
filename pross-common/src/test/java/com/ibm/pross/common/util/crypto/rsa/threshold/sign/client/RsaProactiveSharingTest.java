        ...
        for (int ii = 0; ii < numIterations; ii++) {
            ...
            for (int i = 0; i < numServers; i++) {
                ...
                for (int j = 0; j < numServers; j++) {
                    // Random prime larger than the shared secret -> 2Ls
                    BigInteger randPrime = Primes.generatePrime(rsaProactiveSharing.getBigR().multiply(L).multiply(BigInteger.valueOf(2)).bitLength() + 1);
                    assertTrue(randPrime.isProbablePrime(100));
                    BigInteger result = Polynomials.interpolateComplete(shamirShares, numServers, threshold, j, randPrime).divide(L);

                    if (j == 0) {
                        assertEquals(additiveShares.get(i).getY(), result);
                    } else {
                        assertNotEquals(additiveShares.get(i).getY(), result);
                    }
                }
            }
        }
